...
# what it depends on (the harness, the platform's lib/, its engine, and mk/), so
...
if [ "${IMG:-1}" = 0 ]; then
	...
	rm -f "$_probe"
	if [ ! -f "$_builder" ]; then
		echo "x-make: no image writer at $_builder (not a checkout) -- the suite boots from source" >&2
	elif [ "$_keeps_printer" = 0 ]; then
		echo "x-make: the platform moves a REPL printer it does not own (x-lang#655) -- the suite boots from source" >&2
	elif X_BIN="$X_BIN" sh "$_builder" "$LANG_LIB" "$BUNDLE/tests/lib/.images" "$BUNDLE/mk"; then
		X_IMG_DIR="$BUNDLE/tests/lib/.images"; export X_IMG_DIR
	else
		echo "x-make: no state image -- the suite boots from source" >&2
	fi
fi
